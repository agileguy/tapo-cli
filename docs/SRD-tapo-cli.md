...
- **FR-39b:** Other camera config knobs (HDR, noise cancelling, auto-track, recording-to-SD enable) are **deferred to Phase 2+**. v1 ships only `--image-flip` and `--timezone`.
...
| `motion` | Motion-detection enable / disable / status / history |
...
| `set` | Image flip, timezone (Phase 1 set-flag set; more in Phase 2) |
...
[
  {"alias":"office-cam","mac":"AA:BB:CC:DD:EE:03","cache_path":"/Users/dan/.config/tapo-cli/.tokens/AABBCCDDEE03.json","mtime":"2026-04-28T18:02:14Z","expires_at":"2026-05-28T18:02:14Z","bytes":412,"pytapo_version":"3.4.13","cloud_account":true,"camera_account":true},
  {"alias":"front-door","mac":"AA:BB:CC:DD:EE:01","cache_path":"/Users/dan/.config/tapo-cli/.tokens/AABBCCDDEE01.json","mtime":"2026-04-28T17:42:11Z","expires_at":null,"bytes":412,"pytapo_version":"3.4.13","cloud_account":true,"camera_account":true}
]
...
- **Motion-event video clip downloads.** Vendor-specific binary protocol; pytapo has experimental support but it's brittle. **Low priority not impossible** — a future SRD may revisit if the protocol stabilizes.
...
- **Doorbell event-stream subscription.** Doorbell-press events SHALL surface in `motion history` output (FR-25 with `event_type: "doorbell-press"`) but a long-running event-subscription verb is deferred. Use the cron + `motion history --since` idiom.
...
- **Camera-account creation from the CLI.** **Deferred — protocol exists, low priority.** TP-Link's Tapo app uses a documented HTTPS verb to create a camera account on a device; pytapo could in principle expose it. The reason this is deferred rather than impossible is that the create-camera-account flow is rare enough (run once per device, ever) that the manual app step is acceptable. A future SRD may add `tapo-cli auth setup-camera-account` if operators ask for it.
...
| 8 | **Doorbell event subscription** | Deferred. v1 surfaces doorbell-press events through `motion history --event-type doorbell-press`. Long-running subscription verb is a future SRD. |
...
| 12 | **Motion-clip download** | Deferred, possibly never. Vendor binary protocol; pytapo's support is experimental. |
...
| 19 | **Camera-account creation** | Out of scope. v1 consumes camera accounts; Tapo-app operation creates them. No `tapo-cli auth setup-camera-account` verb. |
...
### 16.2 Phase 2 — PTZ, Presets, Alarm, Audio, OSD (1-2 weeks)
...
- Verb: `set --image-flip` and `set --timezone`
...
There is **no Phase 4 deliverable** in this SRD. Candidates for a future SRD if the need arises: long-running ONVIF event subscription, motion-clip download, HLS transcoding shim, doorbell battery-mode workaround (if TP-Link ever opens it), HDR/noise-cancelling/auto-track set-flags. None are committed.
